...
export default function SimpleChat() {
  ...
  // 根据是否在 iframe 中动态调整聊天窗口样式
  const chatWindowStyle = {
    position: isInIframe ? "relative" : "fixed",
    right: isInIframe ? "auto" : 18,
    bottom: isInIframe ? "auto" : 18,
    width: isInIframe ? "100%" : (isExpanded ? "min(1100px, calc(100vw - 40px))" : "min(430px, calc(100vw - 24px))"),
    height: isInIframe ? "100%" : (isExpanded ? "min(920px, calc(100dvh - 40px))" : "min(820px, calc(100dvh - 24px))"),
    maxWidth: isInIframe ? "none" : "calc(100vw - 24px)",
    background: "#f9fafb",
    borderRadius: isInIframe ? 0 : 18,
    overflow: "hidden",
    border: isInIframe ? "none" : "1px solid #e5e7eb",
    boxShadow: isInIframe ? "none" : "0 24px 60px rgba(0,0,0,.24)",
    zIndex: 2147483645,
    display: isOpen || isInIframe ? "flex" : "none",
              ...
              onClick={() => {
                if (isInIframe) {
                  ...
                  window.parent.postMessage(
                    { source: "cyberhome-simplechat", type: isExpanded ? "chat:collapse" : "chat:expand" },
                    "*" // 可替换为你的 Shopify 域名如 "https://cyberhome.app"
                  );
                }
                setIsExpanded((v) => !v);
              }}
